...
* it subclasses :class:`~qte_shared.strategies.strategy_base.SignalStrategy`, so it has one
...
* the decision reads only ``candles``, which never contains a future bar;
...
class EmaAtrBreakout(SignalStrategy):
    ...
    def long(self, candles: pandas.DataFrame, context: StrategyContext) -> IntentResult:
        return self._entry(candles, context, direction=1)

    def short(self, candles: pandas.DataFrame, context: StrategyContext) -> IntentResult:
        return self._entry(candles, context, direction=-1)

    # ── Exits ─────────────────────────────────────────────────────────

    def tp1(self, candles: pandas.DataFrame, context: StrategyContext) -> IntentResult:
        """Handled by the bracket the entry carried; nothing to decide per bar."""
        return None

    def tp2(self, candles: pandas.DataFrame, context: StrategyContext) -> IntentResult:
        """Likewise — ``tp2`` travels with the entry."""
        return None

    def sl(self, candles: pandas.DataFrame, context: StrategyContext) -> IntentResult:
    ...
    def _entry(
        self, candles: pandas.DataFrame, context: StrategyContext, *, direction: int
    ) -> IntentResult:
        ...
        fast_average = ema(candles["close"], fast_length)
        slow_average = ema(candles["close"], slow_length)
        trend_average = ema(candles["close"], trend_length)
        atr_series = atr(candles, atr_length)
        ...
        if any(pandas.isna(series.iloc[-1]) for series in latest.values()):
            return None
        ...
        if direction > 0:
            crossed = bool(crossover(fast_average, slow_average).iloc[-1]) and close > float(
                trend_average.iloc[-1]
            )
        else:
            crossed = bool(crossunder(fast_average, slow_average).iloc[-1]) and close < float(
                trend_average.iloc[-1]
            )
        if not crossed:
            return None

        risk_distance = atr_value * atr_multiple
        stop_price = close - direction * risk_distance
        take_profit_1 = close + direction * risk_distance * min_rr
        take_profit_2 = close + direction * risk_distance * min_rr * 2
        ...
        return SignalIntent(
            ...
            indicators={
                "close": close,
                "ema_fast": round(float(fast_average.iloc[-1]), 5),
                "ema_slow": round(float(slow_average.iloc[-1]), 5),
                "ema200_filter": round(float(trend_average.iloc[-1]), 5),
